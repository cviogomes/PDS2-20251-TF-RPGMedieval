...
int main() {
    ...
    int escolha_cap1 = fazerEscolha("Os gritos dos inocentes cortam o ar, mas os suprimentos em sua mao podem garantir sua sobrevivencia. O que voce prioriza?", {"Salvar os Civis", "Proteger Recursos"});
    if (escolha_cap1 == 1) {
    ...
    if (!jogador->estaVivo()) {
        narrativePrint("Roland", "Levante-se, heroi. Ainda nao e' sua hora.");
    ...
    int escolha_c3 = fazerEscolha("Voce encontra uma fortaleza rebelde. O que voce faz?", {"Ajudar os Rebeldes", "Invadir o acampamento sozinho"});
